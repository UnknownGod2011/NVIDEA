...
The facade owns exact approval resume + execution so consequential actions cannot bypass terminal evidence publication. It also owns approval rearm and cancellation. Ambiguous crash reconciliation is explicitly non-verifying: trusted host post-state recovery can complete durable state without manufacturing the normal structural terminal evidence required for judge-green status.
...
Validation/evidence:
- Re-read `progress.md` completely and re-audited the current repository tree, `BrowserHostRuntime`, `BrowserDurableActionRuntime`, `BrowserProductRuntime`, and existing verification runtime tests before implementation.
- Identified a fail-closed gap in legacy/migrated ambiguous reconciliation: although ambiguous completion never publishes green evidence, an already-existing protected browser-verification receipt could theoretically survive that transition if the job predates the normal admission-clear lifecycle.
- Hardened `CompleteAmbiguousRunningWithoutVerificationAsync` to clear protected browser-verification evidence before committing the durable ambiguous-completion transition. If protected evidence cannot be cleared, durable reconciliation is not committed; the job remains explicitly recoverable instead of presenting stale judge-green evidence.
- The method remains deliberately non-publishing after reconciliation. A trusted post-state proof can repair durable job state but cannot mint normal last-mile browser verification evidence.
...
- Ambiguous crash reconciliation is explicitly non-verifying and now also invalidates any existing protected browser receipt before durable reconciliation. Clear failure prevents the reconciliation commit, avoiding stale-green evidence.
...
- A production-host integration test is still needed after composition to prove clear-before-admission, completed-side-effect/no-replay behavior, and ambiguous-reconciliation stale-receipt invalidation end to end.
