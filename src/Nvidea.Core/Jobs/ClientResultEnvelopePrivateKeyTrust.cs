...
/// <summary>
/// Single trust boundary for the client RSA private identity used to unwrap protected remote-research
/// result data keys. The key must be bounded private RSA material of at least 2048 bits and must
/// prove the exact OAEP-SHA256 capability used by the result protocol.
/// </summary>
public static class ClientResultEnvelopePrivateKeyTrust
{
    ...
    public static RSA CreateValidatedRsa(string pem)
    {
        ...
        {
            throw new InvalidOperationException("The client result-envelope private key is missing or invalid.");
        }
        ...
        try
        {
            ...
            catch (Exception exception) when (exception is CryptographicException or ArgumentException)
            {
                throw new InvalidOperationException(
                    "The client result-envelope private key is not valid RSA private-key PEM.",
                    exception);
            }

            if (rsa.KeySize < MinimumRsaBits)
                throw new InvalidOperationException("The client result-envelope RSA private key must be at least 2048 bits.");
    ...
    private static void ProvePrivateOaepSha256Capability(RSA rsa)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    "The client result-envelope key must contain usable RSA private material for OAEP-SHA256 decryption.",
                    exception);
            }
            ...
            {
                throw new InvalidOperationException(
                    "The client result-envelope RSA private key failed the OAEP-SHA256 capability proof.");
            }
        }
        catch (Exception exception) when (exception is CryptographicException or ArgumentException)
        {
