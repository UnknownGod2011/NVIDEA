...
public static class NebiusResearchDeploymentPreflight
{
    ...
    private static readonly string[] RequiredSecretEnvironmentVariables =
    {
        ...
        WorkerResultSigningPrivateKeyEnvironmentVariable
    };
    ...
    {
        if (string.IsNullOrWhiteSpace(workerImage) || workerImage.Length > 2048 || workerImage.Any(char.IsControl) || workerImage.Any(char.IsWhiteSpace))
        ...
        if (marker <= 0 || marker + 8 + 64 != workerImage.Length)
            throw new InvalidOperationException("Live Nebius research requires a digest-pinned worker image in registry/path@sha256:<64-hex> form; mutable tags are not accepted by the preflight.");
    ...
    public static void ValidateMysteryBoxSecretReference(string environmentVariableName, NebiusMysteryBoxSecretRef secretRef)
    {
        ...
        if (secretId is null && versionId is null)
            throw new InvalidOperationException($"Worker credential '{environmentVariableName}' must reference a MysteryBox secret or secret version.");
        if (secretId is not null && !IsNebiusResourceId(secretId, "mbsec-"))
            throw new InvalidOperationException($"Worker credential '{environmentVariableName}' has an invalid MysteryBox secret id.");
        if (versionId is not null && !IsNebiusResourceId(versionId, "mbsecver-"))
            throw new InvalidOperationException($"Worker credential '{environmentVariableName}' has an invalid MysteryBox version id.");
        if (versionId is not null && secretId is null)
            throw new InvalidOperationException($"Worker credential '{environmentVariableName}' must include its MysteryBox secret id when pinning a version id.");
    }
    ...
    public static void ValidateObjectStorageAlignment(NebiusResearchDispatchOptions dispatchOptions, NebiusObjectStorageTransportAlignment alignment)
    {
        ...
        if (!string.Equals(bucket, source, StringComparison.Ordinal))
            throw new InvalidOperationException("Native Object Storage bucket must exactly match the Serverless research transport volume source.");
        ...
        if (!string.Equals(clientPrefix, mountedSourcePath, StringComparison.Ordinal))
            throw new InvalidOperationException("Native Object Storage prefix must exactly match the Serverless research transport volume SourcePath.");
    }
    ...
    public static void ValidateObjectStorageAlignment(NebiusResearchDispatchOptions dispatchOptions, NebiusObjectStorageClientOptions objectStorageOptions)
    {
        ...
        ValidateObjectStorageAlignment(dispatchOptions, new NebiusObjectStorageTransportAlignment(objectStorageOptions.Bucket, objectStorageOptions.Prefix));
    }
    ...
    private static string? NormalizeMysteryBoxId(string? value)
    {
        ...
        if (normalized.Length > 128 || normalized.Any(char.IsControl) || normalized.Any(char.IsWhiteSpace)) return string.Empty;
        return normalized;
    }

    private static bool IsNebiusResourceId(string value, string prefix)
    {
        if (string.IsNullOrEmpty(value) || !value.StartsWith(prefix, StringComparison.Ordinal) || value.Length <= prefix.Length) return false;
        return value.AsSpan(prefix.Length).ToArray().All(static character => char.IsAsciiLetterOrDigit(character) || character is '-' or '_');
    }

    private static string NormalizeBucket(string value)
    {
        if (string.IsNullOrWhiteSpace(value) || value.Length > 63 || value.Any(char.IsControl) || value.Contains('/') || value.Contains('\\'))
    ...
    {
        if (value.Length > 128 || value.Any(char.IsControl) || value.Contains('\\')) throw new InvalidOperationException($"{description} is invalid.");
        var normalized = value.Trim().Trim('/');
        if (normalized.Contains("..", StringComparison.Ordinal) || normalized.Split('/', StringSplitOptions.RemoveEmptyEntries).Any(static segment => segment is "." or ".."))
