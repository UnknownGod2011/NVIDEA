...
/// <summary>
/// Single trust boundary for the worker RSA public identity used to wrap protected research
/// work-item data keys. The key must be bounded, public-only RSA material of at least 2048 bits and
/// must prove the exact OAEP-SHA256 encryption capability used by the remote research protocol.
/// </summary>
public static class WorkerEnvelopePublicKeyTrust
{
        ...
        try
        {
            ...
            catch (Exception exception) when (exception is CryptographicException or ArgumentException)
            {
                throw new InvalidOperationException(
                    "The worker envelope public key is not valid RSA public-key PEM.",
                    exception);
            }

            if (rsa.KeySize < MinimumRsaBits)
                throw new InvalidOperationException("The worker envelope RSA public key must be at least 2048 bits.");
    ...
    private static void RejectPrivateMaterial(RSA rsa)
    {
        ...
        throw new InvalidOperationException(
            "The worker envelope encryption key must be public-only RSA material; private key material is not permitted.");
    }
    ...
    private static void ProvePublicOaepSha256Capability(RSA rsa)
    {
        ...
        try
        {
            ...
            if (wrapped.Length == 0)
                throw new InvalidOperationException("The worker envelope RSA public key failed the OAEP-SHA256 capability proof.");
        }
        catch (Exception exception) when (exception is CryptographicException or ArgumentException)
        {
            throw new InvalidOperationException(
                "The worker envelope key must contain usable RSA public material for OAEP-SHA256 encryption.",
                exception);
        }
        finally
        {
