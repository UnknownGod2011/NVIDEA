...
public sealed class PersistentBrowserRedirectIntegrationTests
{
    ...
    public async Task CredentialBearingRedirect_IsStoppedBeforeDestinationRequest()
    {
        ...
        try
        {
            ...
            session = await PersistentBrowserContextFactory.LaunchAsync(
                ...
                stateDirectory,
                server.UriFor("/safe"),
                new PlaywrightBrowserDriverOptions(ActionTimeoutMilliseconds: 5_000),
                headless: true);

            var destinationBefore = server.Count("/forbidden-destination");
            var redirectUri = server.UriFor("/redirect-to-credential-url");

            await Assert.ThrowsAnyAsync<Exception>(() => session.Context.Pages.Single().GotoAsync(
    ...
    private static bool IntegrationEnabled() =>
        string.Equals(Environment.GetEnvironmentVariable("NVIDEA_RUN_PLAYWRIGHT_INTEGRATION"), "1", StringComparison.Ordinal);

    private static string CreateStateDirectory() =>
        Path.Combine(Path.GetTempPath(), "nvidea-browser-integration", Guid.NewGuid().ToString("N"));
    ...
    private static void TryDeleteDirectory(string path)
    {
        ...
        {
            // Chromium can briefly retain profile handles on Windows. Test correctness is determined
            // by transport observations, not best-effort temp cleanup.
        }
    }
    ...
    private sealed class LoopbackHttpServer : IAsyncDisposable
    {
        ...
        public static Task<LoopbackHttpServer> StartAsync()
        {
            ...
            listener.Start();
            return Task.FromResult(new LoopbackHttpServer(listener));
        }
        ...
        private async Task HandleAsync(TcpClient client)
        {
            using (client)
            {
                try
                {
                    ...
                    var path = parts.Length >= 2 && Uri.TryCreate(parts[1], UriKind.RelativeOrAbsolute, out var target)
                        ? (target.IsAbsoluteUri ? target.PathAndQuery : parts[1])
                        : "/";
                    ...
                    {
                        var location = $"http://nvidea-test-secret@127.0.0.1:{Port}/forbidden-destination";
                        await WriteResponseAsync(stream, "302 Found", "redirect", $"Location: {location}\r\n").ConfigureAwait(false);
                    }
                    else
                    {
                        await WriteResponseAsync(stream, "200 OK", "ok", string.Empty).ConfigureAwait(false);
                    }
                }
                catch (IOException)
                {
                    // Browser cancellation can close a socket while the test server is responding.
                }
            }
        }

        private static async Task WriteResponseAsync(NetworkStream stream, string status, string body, string extraHeaders)
        {
