...
public sealed class DemoPackageValidatorTests
{
    ...
    [
        ...
        [nameof(SessionEvidenceKind.BrowserPostStateVerified)],
    ...
    [Fact]
    public void Checked_in_manifest_uses_exact_production_session_evidence_sequence()
    {
        ...
        var parsed = declared.Select(name =>
        {
            Assert.True(Enum.TryParse<SessionEvidenceKind>(name, ignoreCase: false, out var kind), $"Unknown production session milestone: {name}");
            Assert.True(Enum.IsDefined(kind), $"Undefined production session milestone: {name}");
        ...
        var epoch = DateTimeOffset.Parse("2026-09-22T00:00:00Z");
        var snapshot = new SessionEvidenceSnapshot(parsed.Select((kind, index) =>
            new SessionEvidenceEntry(kind, epoch.AddSeconds(index))).ToArray());
        Assert.Equal(expected, snapshot.Entries.Select(entry => entry.Kind).ToArray());
        Assert.All(expected, kind => Assert.True(snapshot.Contains(kind)));
    }
    ...
    [Fact] public async Task Wrong_session_milestone_is_rejected() { using var f=new ValidatorFixture(); var m=f.CreateManifest(); f.Beat(m,2)["expectedSessionMilestones"]=new[]{nameof(SessionEvidenceKind.NemotronInferenceCompleted)}; Assert.Equal(1,await f.RunAsync(m)); }
    ...
    [Fact] public async Task Architecture_beat_cannot_claim_runtime_milestone() { using var f=new ValidatorFixture(); var m=f.CreateManifest(); f.Beat(m,6)["expectedSessionMilestones"]=new[]{nameof(SessionEvidenceKind.NemotronInferenceCompleted)}; Assert.Equal(1,await f.RunAsync(m)); }
